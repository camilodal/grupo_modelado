...
function descomponer(im) ##devuelve el ycbcr con la cb y cr reducidas
...
function descomponer_inversa(im) ##devuelve el rgb
a=im[1]
b=im[2]
c=im[3]
...
function transforma_8x8!(matriz)
	for i in 1:div(size(matriz)[1], 8) 
	for j in 1:div(size(matriz)[2], 8)
		vista=view(matriz, 8*(i-1)+1:8*i, 8*(j-1)+1:8*j)
		dct!(vista)
	end
	end
end

# ╔═╡ a1ce0382-a682-4b82-b930-2337f54bb0aa
function transformada_por_bloques!(im)
	transforma_8x8!(im[1])
	transforma_8x8!(im[2])
	transforma_8x8!(im[3])
end

# ╔═╡ c5088eb2-0ebc-4f2b-8670-1a4396c1a519
function inversa_transforma_8x8!(matriz)
	for i in 1:div(size(matriz)[1], 8) 
	for j in 1:div(size(matriz)[2], 8)
		vista=view(matriz, 8*(i-1)+1:8*i, 8*(j-1)+1:8*j)
		idct!(vista)
...
# ╔═╡ b4254643-3fd8-439f-8abc-a574b88d15d0
function inversa_transformada_por_bloques!(im)
	inversa_transforma_8x8!(im[1])
	inversa_transforma_8x8!(im[2])
	inversa_transforma_8x8!(im[3])
end

# ╔═╡ edf5805d-0032-4cff-a508-eb3e949220e4
md""" ### Prueba de funciones"""

# ╔═╡ f47055c2-7689-49ba-8322-762db7d35c37
imagen1= load("bolitas.bmp")

# ╔═╡ 8c314a3f-564b-4bbb-9a32-1e341167e51c
im2=completar_tam_16(imagen1)

# ╔═╡ 5791c6b1-0924-41ee-9d1b-94eafef10938
descomponer(im2)

# ╔═╡ 64ea63e1-d791-49a3-bf62-9d8c1a835969
descomponer_inversa(descomponer(im2))

# ╔═╡ 799bfcfd-d332-4779-88cf-fc66db57a875
a=descomponer(im2)

# ╔═╡ cd6ec689-c1f2-42f2-b6ae-8ad5816b1c15
transformada_por_bloques!(a)

# ╔═╡ 658f609d-c3c2-4b31-ad2f-6dd98208a56a
a
