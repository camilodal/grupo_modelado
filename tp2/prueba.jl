...
# ╔═╡ 37f4fe4e-9510-11ee-23b8-673fa0a44625
using Images, FFTW, StatsBase

# ╔═╡ 09964af5-a4d9-4533-a05d-30c0f2a3b063
md""" Primera Parte"""

# ╔═╡ eb3220e9-e56f-4622-9221-1a6355c36d20
begin
	function completar_tam_16(im)
		...
		if(resto1 == 0 &&resto2 == 0)
			...
			mitadizq_1 = div(16-resto1, 2)
			mitadder_1= 16-resto1-mitadizq_1
			mitadizq_2 = div(16-resto2, 2)
			mitadder_2= 16-resto1-mitadizq_2
			nueva_im = fill(RGB(0, 0, 0), tam1+16-resto1, tam2+16-resto2)
			nueva_im[16-resto1+1:16-resto1+tam1, 16-resto2+1:16-resto2+tam2]=copy(im)
			return(nueva_im)
		end
	end
	...
	function descomponer(im)
		...
		intensidad = imagen1_channelview[1, :, :]
		cb = imagen1_channelview[2, :, :]
		nueva_im_cb = fill(Float64(0), div(size(cb)[1], 2), div(size(cb)[2], 2)) 
		cr= imagen1_channelview[3, :, :]
		nueva_im_cr= fill(Float64(0), div(size(cr)[1], 2), div(size(cr)[2], 2)) 
	...
	function descomponer_inversa(im)
		...
		for i in 1:size(b,1)
		for j in 1:size(b,2)
			nueva_im_b[2*i-1,2*j-1]= b[i, j]+ 128
			nueva_im_b[2*i-1,2*j]= b[i, j]+128
...
# ╔═╡ 5ea39cea-1a8f-4528-bf0d-19d0dc555a02
md""" Transformada """

# ╔═╡ 920c8e07-f441-4f2b-9334-94098873f61a
begin 
...
# ╔═╡ 1833bf25-b984-4f63-94e2-7828dd1f803c
md""" Cuantizacion """ 

# ╔═╡ 9688e5ec-2a12-4656-9de7-ba127f3f33d7
begin 
	function cuantizar(M)
...
# ╔═╡ a0b0aab4-1bca-4f31-94bc-c93666800f1f
md""" Compresion """ 

# ╔═╡ 9fb9e280-9159-44c6-94d2-74f79d3aaee7
begin
...
# ╔═╡ 1bd87bc1-9e81-4182-a0c8-d43c08dcb10b
begin 
	function compresion_bloques_8x8(m)
		vect = []
		orden_zigzag1 = orden_zigzag()
		for indexs in orden_zigzag1
	...
	function inversa_compresion_8x8(M,V)
		orden_zigzag1 = orden_zigzag()
		for i in 1:64 
			M[orden_zigzag1[i][1],orden_zigzag1[i][2]] = V[i]
		end
	end 
...
# ╔═╡ 5e35df3a-227a-4024-b6c2-f7d399911b91
md""" Lectura y Guardado """

# ╔═╡ 2bd2f99a-9072-4337-ba2f-0a6a543823d9
begin 
	function leer_M(nombre::String)
		...
		while(!termino)
			if suma_parcial == 64 
				...
				reps = zeros(Int8,0)
				vals = zeros(Int8,0)
				if(j == bloq_cols2 && i < bloq_fils2)
					j = 1
					i += 1
				elseif(j == bloq_cols2 && i == bloq_fils2)
		...
		M2 = inversa_compresion(A,bloq_fils2,bloq_cols2)
		...
		close(io)
		M3 = inversa_compresion(A,bloq_fils2,bloq_cols2)
		return [M1,M2,M3]
	end
	function guardar_M(M,n,m,nombre)
...
# ╔═╡ 734ed826-7815-4026-a4bb-c34f14882bb9
imagen1 = load("Meisje_met_de_parel.jpg")

# ╔═╡ f784865e-7f50-492e-bfa2-2557bec8e8ab
begin 
	function Primer_Paso(path_img)
		img = load(path_img)
		...
		img_cuantizada1 = cuantizar(img_descomp[1])
		img_cuantizada2 = cuantizar(img_descomp[2])
		img_cuantizada3 = cuantizar(img_descomp[3])
		
		n,m = size(img_cuantizada1)
	
		intensidad = compresion_matriz(img_cuantizada1)
		cb = compresion_matriz(img_cuantizada2)
		cr = compresion_matriz(img_cuantizada3)
		nombre = path_img[begin:end-4]
		guardar_M([intensidad,cb,cr],n,m,nombre)
		return nombre
	end
...
# ╔═╡ 665270f4-0c49-45a0-8920-9eb36b762aed
#Segundo_paso("paisaje")

# ╔═╡ 3512c85e-4f7c-4cb5-9bbe-f351634b53c5
Primer_Paso("paisaje.bmp")
