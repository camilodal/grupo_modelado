...
begin
	df = CSV.read("WHO-COVID-19-global-data.csv",DataFrame;header=1)
	df = filter(row->row["Country"]=="Argentina",df)
	df = df[56:1336,:]
end

# ╔═╡ 7c5e7932-3f6c-4374-82fb-21b216294b20
plot(df[:,"Date_reported"],df[:,"New_cases"])

# ╔═╡ 499fbe72-5544-4927-9a96-2cf325918ede
begin
	df.semana = zeros(Int,length(df[:,"Date_reported"]))
	for i in 1:length(df[:,"Date_reported"])
		df.semana[i] = convert(Int,div(Dates.value(df[i,"Date_reported"]-df[1,"Date_reported"]),7))
	end
	groupby(df, :semana)
	casosSemana = combine(groupby(df, :semana),:New_cases => sum => "casos",:Cumulative_cases => maximum => "acumulados")
	plot(casosSemana.semana,casosSemana.casos)
end

# ╔═╡ 5999ab09-2a7b-400b-bd77-f0a072f08ca0
groupby(df, :semana)

# ╔═╡ e14fd28c-90d6-40a8-9ffd-493590387988
begin
	valles = [1]
	ancho = 4
	for i in (ancho+1):(casosSemana.semana[end]-ancho+1)
		minV = casosSemana.casos[i-ancho]
		for j in (i-ancho):(i-1)
			if casosSemana.casos[j] < minV
				minV = casosSemana.casos[j]
			end
		end

		for j in (i+1):(i+ancho)
			if casosSemana.casos[j] < minV
				minV = casosSemana.casos[j]
			end
		end
		...
		if casosSemana.casos[i]<minV
			push!(valles,i)
		end
	end
	print(valles)
	primeraOla = casosSemana[valles[1]:valles[2],:]
	primeras2Olas = casosSemana[valles[1]:valles[3],:]
end

# ╔═╡ 9105da77-79a0-4b5f-b026-854794af74db
begin
	xs,ys,labs = [],[], Matrix{String}(undef,1,length(valles)-1)
	for i in 1:(length(valles)-1)
		#print(i)
		#push!(plots,plot(casosSemana.semana[valles[i]:valles[i+1]],casosSemana.casos[valles[i]:valles[i+1]]))
		push!(xs,casosSemana.semana[valles[i]:valles[i+1]])
		push!(ys,casosSemana.casos[valles[i]:valles[i+1]])
		labs[i] = "Ola $i"
	end
	plot(xs,ys, label=labs)
end

# ╔═╡ d70a947e-e9b9-44da-b10c-624b068b59fb
plot(primeraOla.semana,primeraOla.casos, label="Ola 1")

# ╔═╡ c34e0a12-ae2d-4965-ba17-4b114803670f
plot([primeras2Olas.semana[1:valles[2]],primeras2Olas.semana[valles[2]:valles[3]]],[primeras2Olas.casos[1:valles[2]],primeras2Olas.casos[valles[2]:valles[3]]], label=["Ola 1"  "Ola 2"])

# ╔═╡ 84f4a803-ef50-432d-a270-eb2f8477fa7d
begin 
	olasNormalizadas = [copy(primeraOla), copy(primeras2Olas), copy(casosSemana)]
	for i in 1:3
		olasNormalizadas[i].casos = olasNormalizadas[i].casos ./ 45.5e6
		olasNormalizadas[i].acumulados = olasNormalizadas[i].acumulados ./ 45.5e6
	end
	plot([olasNormalizadas[2].semana[1:valles[2]],olasNormalizadas[2].semana[valles[2]:valles[3]]],[olasNormalizadas[2].casos[1:valles[2]],olasNormalizadas[2].casos[valles[2]:valles[3]]], label=["Ola 1"  "Ola 2"])
end

# ╔═╡ 6a76d75c-4726-45fa-a70d-258784b8ef48
plot([olasNormalizadas[2].semana[1:valles[2]],olasNormalizadas[2].semana[valles[2]:valles[3]]],[olasNormalizadas[2].acumulados[1:valles[2]],olasNormalizadas[2].acumulados[valles[2]:valles[3]]], label=["Ola 1"  "Ola 2"])
...
# ╔═╡ dc0f27c9-9ac2-478e-9655-95d5cc775029
...
# ╔═╡ 93280261-1038-409c-830b-6259449bfb4a
function SEIR!(du,u,p,t)
	du[1] = -p[1]*u[1]*u[3]
	du[2] = p[1]*u[1]*u[3]-p[2]*u[2]
	du[3] = p[2]*u[2]-p[3]*u[3]
	du[4] = p[3]*u[3]
end

# ╔═╡ 0a12cf14-1e1e-4b9f-9eb9-625d51f560ab
function getNISEIR(sol,n)
	nI = zeros(n)
	for i in 1:n
		nI[i] = sol.(i,idxs=2)*sol.prob.p[2]
	end
	return nI
end 

# ╔═╡ 2596d7c6-947d-401b-9589-229e196856ad
function SEIRS!(du,u,p,t)
	du[1] = -p[1]*u[1]*u[3]+p[4]*u[4]
	du[2] = p[1]*u[1]*u[3]-p[2]*u[2]
	du[3] = p[2]*u[2]-p[3]*u[3]
	du[4] = p[3]*u[3]-p[4]*u[4]
end

# ╔═╡ a652617c-cef5-4f14-bde7-54fb34f26acf
function getNISEIRS(sol,n)
	nI = zeros(n)
	for i in 1:n
		nI[i] = sol.(i,idxs=2)*sol.prob.p[2]
	end
...
# ╔═╡ faf66444-4414-47c5-b6d1-501afec44ccf
function GenLoss(datos, index, getNI)
	lossF = function(sol)
		if any((s.retcode != :Success for s in sol))
			return Inf
		else
			loss = 0
			n = length(datos.semana)
			nI = getNI(sol,n)
			for i in 1:n
				loss += (nI[i]-datos.casos[i])^2
			end
			#if grad == 2
			#	for i in 2:n
			#		loss += ((sol.(i,idxs=index)-sol.(i-1,idxs=index))-#(datos.acumulados[i]-datos.acumulados[i-1]))^2
			#	end
			#end 
			return loss
		end
...
function Resolver(datos, modelo, lossF, ufq, pfq, q0, lb, ub, getNI)
	eps = 1/45.5e6
	print(ufq(q0))
	problema = ODEProblem(modelo,ufq(q0),(0,datos.semana[end]),abstol=1e-7, reltol=1e-7)
	func = build_loss_objective(problema, AutoTsit5(Rosenbrock23()), lossF,Optimization.AutoFiniteDiff(), prob_generator = (prob,q) -> remake(prob,u0=ufq(q),p=pfq(q)))
	problemaOpt = OptimizationProblem(func, q0, lb=lb, ub=ub)
	optimo = solve(problemaOpt,SAMIN(),maxiters=1000000)
	optsol = solve(remake(problema,u0=ufq(optimo),p=pfq(optimo)))
	println(optimo)
	println(getNI)
	plot(datos.semana,getNI(optsol,length(datos.semana)))
	#plot(optsol,idxs = idx)
	scatter!(datos.semana,datos.casos)
end

# ╔═╡ fa534c72-985d-4e3f-be8f-6e28ab9f6322
Resolver(olasNormalizadas[1], SIR! , GenLoss(olasNormalizadas[1], 2, getNISIR), (q)->[1-q[1],q[1],0],(q)->[q[2],
	q[3]],[100/45.5e6,1/0.5,1/1],[1/45.5e6,0,0],[1000/45.5e6,21,14],getNISIR)

# ╔═╡ b610ed68-9947-49cd-9c3d-34b10c918a52
Resolver(olasNormalizadas[1], SEIR! , GenLoss(olasNormalizadas[1], 2, getNISEIR), (q)->[1-q[1],0,q[1],0],(q)->[q[2], q[3],q[4]] ,
	[100/45.5e6,1/0.5,7/3, 1/1],[1/45.5e6, 1, 0.5, 0],[1000/45.5e6,28,21,14], getNISEIR)

# ╔═╡ a88632d6-bd8e-4705-b96b-6375e1ad8ddd
Resolver(olasNormalizadas[1], SEIRS! , GenLoss(olasNormalizadas[1], 2, getNISEIRS), (q)->[1-q[1],0,q[1],0],(q)->[q[2], q[3],q[4], q[5]] ,
	[100/45.5e6,1/0.5,7/3, 1/1, 0.1],[1/45.5e6, 1, 0.5, 0,0],[1000/45.5e6,28,21,14, 2], getNISEIRS)
...
Resolver(olasNormalizadas[2], SIR! , GenLoss(olasNormalizadas[2], 2, getNISIR), (q)->[1-q[1],q[1],0],(q)->[q[2],
	q[3]],[10/45.5e6,1/0.5,1/1],[1/45.5e6,0,0],[1000/45.5e6,21,7/2],getNISIR)

# ╔═╡ 862f9b72-e585-4ee1-89ee-c9b535946925
Resolver(olasNormalizadas[2], SEIR! , GenLoss(olasNormalizadas[2], 2, getNISEIR), (q)->[1-q[1],0,q[1],0],(q)->[q[2], q[3],q[4]] ,
	[100/45.5e6,1/0.5,7/3, 1/1],[1/45.5e6, 1, 0.5, 0],[1000/45.5e6,28,21,14], getNISEIR)

# ╔═╡ d3d97788-7173-4d7c-aa5c-097a0bc591be
Resolver(olasNormalizadas[2], SEIRS! , GenLoss(olasNormalizadas[2], 2, getNISEIRS), (q)->[1-q[1],0,q[1],0],(q)->[q[2], q[3],q[4], q[5]] ,
	[10/45.5e6,1/0.5, 7/3, 1/1, 0.2],[1/45.5e6, 1, 0.5, 0,0],[1000/45.5e6,21,14,14, 4], getNISEIRS)

# ╔═╡ b889c925-61df-4750-8de2-2140c89672ed
Resolver(olasNormalizadas[3], SEIRS! , GenLoss(olasNormalizadas[3], 2, getNISEIRS), (q)->[1-q[1],0,q[1],0],(q)->[q[2], q[3],q[4], q[5]] ,
	[10/45.5e6,1/0.5, 7/3, 1/1, 0.2],[1/45.5e6, 1, 0.5, 0,0],[1000/45.5e6,21,14,14, 4], getNISEIRS)
...
function SIRln!(du,u,p,t)
	du[1] = -p[1]*u[1]*u[2]/log(exp(1)+u[2])
	du[2] = p[1]*u[1]*u[2]/log(exp(1)+u[2])-p[2]*u[2]
	du[3] = p[2]*u[2]
end

# ╔═╡ 37c4c925-e8f9-403b-9e40-a1020bc062a8
Resolver(olasNormalizadas[1], SIRln! , GenLoss(olasNormalizadas[1], 2, getNISIR), (q)->[1-q[1],q[1],0],(q)->[q[2],
	q[3]],[100/45.5e6,1/0.5,1/1],[1/45.5e6,0,0],[1000/45.5e6,21,14],getNISIR)

# ╔═╡ 4ce68d5d-4093-4b64-89a0-e82412adc9bf
function SEIRln!(du,u,p,t)
	du[1] = -p[1]*u[1]*u[3]/log(exp(1)+u[3])
	du[2] = p[1]*u[1]*u[3]/log(exp(1)+u[3])-p[2]*u[2]
	du[3] = p[2]*u[2]-p[3]*u[3]
	du[4] = p[3]*u[3]
end

# ╔═╡ 047b8e12-8edd-4575-8c05-adf2b18e7f1e
Resolver(olasNormalizadas[1], SEIRln! , GenLoss(olasNormalizadas[1], 2, getNISEIR), (q)->[1-q[1],0,q[1],0],(q)->[q[2], q[3],q[4]] ,
	[100/45.5e6,1/0.5,7/3, 1/1],[1/45.5e6, 1, 0.0, 0],[1000/45.5e6,28,210,14], getNISEIR)

# ╔═╡ e20e535f-6789-4804-a1e7-cbdbf54874d7
function SEIRSln!(du,u,p,t)
	du[1] = -p[1]*u[1]*u[3]/log(exp(1)+u[3])+p[4]*u[4]
	du[2] = p[1]*u[1]*u[3]/log(exp(1)+u[3])-p[2]*u[2]
	du[3] = p[2]*u[2]-p[3]*u[3]
	du[4] = p[3]*u[3]-p[4]*u[4]
end

# ╔═╡ 231336fc-1a8b-4264-b7bd-233eed5e3882
Resolver(olasNormalizadas[1], SEIRSln! , GenLoss(olasNormalizadas[1], 2, getNISEIRS), (q)->[1-q[1],0,q[1],0],(q)->[q[2], q[3],q[4], q[5]] ,
	[10/45.5e6,1/0.5, 7/3, 1/1, 0.2],[1/45.5e6, 1, 0.5, 0,0],[1000/45.5e6,21,28,28, 4], getNISEIRS)

# ╔═╡ 7521998e-16b9-44bd-9459-92921bdf34b7
Resolver(olasNormalizadas[2], SEIRSln! , GenLoss(olasNormalizadas[2], 2, getNISEIRS), (q)->[1-q[1],0,q[1],0],(q)->[q[2], q[3],q[4], q[5]] ,
	[10/45.5e6,1/0.5, 7/3, 1/1, 0.2],[1/45.5e6, 1, 0.5, 0,0],[1000/45.5e6,28,28,28, 4], getNISEIRS)

# ╔═╡ 398cc44a-eab7-4692-bf0f-1c11de6e2d5d
Resolver(olasNormalizadas[3], SEIRSln! , GenLoss(olasNormalizadas[3], 2, getNISEIRS), (q)->[1-q[1],0,q[1],0],(q)->[q[2], q[3],q[4], q[5]] ,
	[10/45.5e6,1/0.5, 7/3, 1/1, 0.2],[1/45.5e6, 1, 0.5, 0,0],[1000/45.5e6,14,14,14, 4], getNISEIRS)
