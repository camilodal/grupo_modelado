...
datos=CSV.read("WHO-COVID-19-global-data.csv", DataFrame);
...
begin
	...
	# Luego, agrupamos por la columna 'Week'.
...
md"""
!!! b
	Filtramos los datos de Argentina, agrupando por semana y quedandonos con la fecha desde el 29/02, 5 días antes de que empiece a haber positivos. Graficamos"""

# ╔═╡ c9f88c86-3b56-4302-9da7-eab9e28ae26a
plot(datos_argentina.Week, datos_argentina.Cumulative_cases, title="Casos acumulados", legend=false, ylabel="Cantidad de casos", xlabel= "Semana")

# ╔═╡ 5f6ed769-c991-4d56-922c-ab151bbc371c
plot(datos_argentina.Week, datos_argentina.New_cases, title="Casos por día", legend=false, ylabel="Cantidad de casos", xlabel= "Semana")

# ╔═╡ bcaacec3-27c2-4654-8b0d-9d17609b5f33
md"""
!!! c
	Normalizamos los datos para que se vean como porcentaje y volvemos a graficar"""
...
plot(datos_argentina.Week, datos_argentina.Cumulative_cases, title="Casos acumulados normalizados", legend=false, ylabel="Cantidad de casos", xlabel= "Semana")

# ╔═╡ 75faea2a-8409-415c-9157-82ab19752c68
plot(datos_argentina.Week, datos_argentina.New_cases, title="Casos por día normalizados", legend=false, ylabel="Cantidad de casos", xlabel= "Semana")

# ╔═╡ 13085296-7ee6-45d1-a6c3-77a6fb506f6e
md"""
!!! d
	Separamos en olas y graficamos"""

# ╔═╡ 9f86409b-94a4-439e-8cc8-534a8053b070
begin
	# Crear una columna 'EsMinimoLocal' para marcar mínimos locales.
	datos_argentina.EsMinimoLocal .= false
	# Número de semanas anteriores y posteriores para verificar.
	n_semanas = 3
	for i in (n_semanas+1):(length(datos_argentina.New_cases)-n_semanas-1)
	    semana_actual = datos_argentina.New_cases[i]
	    
	    # Verificar si la semana actual es un mínimo local.
	    if all(semana_actual .< datos_argentina.New_cases[i - n_semanas:i - 1]) && all(semana_actual .< datos_argentina.New_cases[i + 1:i + n_semanas])
	        datos_argentina.EsMinimoLocal[i] = true
	    end
	end
...
begin
	# Encuentra los índices de los 'true' en la columna 'EsMinimoLocal'
	indices_true = findall(datos_argentina.EsMinimoLocal .== true)

	# Divide los datos antes del primer 'true'
	primera_ola_arg = datos_argentina[1:(indices_true[1]), :]
	# Divide los datos entre el primer y el segundo 'true'
	segunda_ola_arg = datos_argentina[1:indices_true[2], :]
end;

# ╔═╡ 1a5b434d-1b47-419b-ae5e-6efd0b53409e
begin
	primera_ola= select(primera_ola_arg, [:Week, :New_cases, :Cumulative_cases, :New_deaths, :Cumulative_deaths])
	plot(primera_ola.Week, primera_ola.New_cases, title="Primera ola (casos por semana)", legend=false, ylabel="Cantidad de casos", xlabel= "Semana")
end


# ╔═╡ 83fc7482-8a46-4842-83cb-d05d6f6d67da
begin
	segunda_ola= select(segunda_ola_arg, [:Week, :New_cases, :Cumulative_cases, :New_deaths, :Cumulative_deaths])
	plot(segunda_ola.Week, segunda_ola.New_cases, title="Segunda ola (casos por semana)", legend=false, ylabel="Cantidad de casos", xlabel= "Semana")
end
...
$$\left\{\begin{array}{rcl}
	\dot{S} &=& -\beta \frac{SI}{N} + \delta R \\ 
	\dot{E} &=& \beta \frac{SI}{N} - \gamma E \\
	\dot{I} &=& \gamma E -\sigma I \\
	\dot{R} &=& \sigma I - \delta R
\end{array}\right.$$ 
"""
...
md""" Primero definimos las funciones que nos devuelvan los valores βSI y γE para cada modelo en particular """ 

# ╔═╡ 930b2249-2b5a-46b3-8e41-24bc4e3d8c0d
begin 
...
# ╔═╡ 68d76ef0-6329-44d9-beae-d72ba050d623
md""" # Implementación de modelos"""

# ╔═╡ 8b5bde68-2466-4603-aa56-7e3b2ac7682b
md""" Ahora definimos la funcion de costo para el modelo SIR. """

# ╔═╡ da818aca-ce13-456c-ac7f-3eca53962c63
begin 
	function func_sir(x,r)
		tot_loss = 0.0
...
# ╔═╡ 572152b0-cd4b-499d-81c0-ed6acfa50a5c
md""" Declaramos el problema de optimizacion usando la funcion de costo previamente definida.
Aclaramos que usamos la primer variable para definir los datos iniciales donde tenemos 

		S₀ = 1 - α, I₀ = α, R₀ = 0 

Para mas comodidad tomamos α como los infectados iniciales y las demas variables para β y σ. 
"""

# ╔═╡ df275ce2-59f3-4e1a-aee2-5f6f3a0fed64
optprob = OptimizationProblem(func_sir,[100/45.5e6,1/0.5,1],lb=[1/45.5e6,0,0],ub=[1000/45.5e6,21,14])
...
begin
	...
	prob_final = ODEProblem(sir!,datos_in,(1,n),p̄[2:3])
	sol_opt    = solve(prob_final,Vern7())
	plot([1:n], getβSIsir(sol_opt, n))
	scatter!([1:n],primera_ola.New_cases)
end

# ╔═╡ 6ea1b642-f01d-4733-a3be-5e044d076e09
md"""
	El gráfico parece ajustarse de una manera razonable a los valores dados. El β obtenido es alrededor de 7.42735 puede llegar a tener sentido para este tramo, ya que el inverso es alrededor de 0.14 que representa el tiempo medio entre contagios, medido en semanas, sin embargo el σ obtenido es 7.26058, de la misma forma su inverso es cercano a 0.14, esto representa el tiempo medio de recuperados en semanas, esto es demasiado bajo conciderando que realmente fueron 2 semanas el tiempo medio de recuperacion, pero como nuestro modelo es muy simple quizas no contempla todo el panorama."""
...
# ╔═╡ a1d55819-8b45-4b71-9391-b140dec22453
md""" Representamos los datos iniciales con 2 variables, fijamos una con los infectados y la otra para variar los Expuestos, luego determinar los susceptibles con estas ultimas, osea: 

	S₀ = 1- I₀ - E₀, E₀ = δ(1-α), I₀ = α, R₀ = 0 

donde α,δ ∈ [0,1], y las demas variables representan los parametros del modelo.
"""
...
begin
	datos_iniciales_seir=[1-p_seir[1]-p_seir[2]*(1-p_seir[1]), p_seir[2]*(1-p_seir[1]),p_seir[1],0]
	prob_final_seir = ODEProblem(seir!,datos_iniciales_seir,(1,42),p_seir[3:5])
...
# ╔═╡ 33731569-c3eb-4af5-80f5-dd532c3b1e63
md""" No se ve una mejora en la función de costos, a pesar de haberse agregado la variable de los expuestos al modelo.

Además, el tiempo medio entre contagios y tiempo medio de recuperación de nuevo dan numeros parecidos al modelo previamente hecho. 

En cuanto al tiempo medio de incubación(γ^{-1}), que es 0.02, es aún más bajo y tiene sentido debido a la naturaleza del virus, pues el modelo nos quiere mostrar que el tiempo que tarda un expuesto en ser un infectado es muy rapido. Sumado a los datos con los que estamos ajustado, que no contemplan el paso intermedio entre los suceptibles e infectados, es decir los expuestos, no termina de ser un parametro que explique lo que realmente esta suceciendo con el tiempo medio de incuvacion del virus."""
...
# ╔═╡ 5032bbca-61e3-4815-b37d-5195d1472b6c
md"""#### Para una ola"""

# ╔═╡ b9bba779-ff31-456c-92dc-b21e805c5c70
function func_seirs_1(x,r)
		tot_loss = 0.0
		n = length(primera_ola.New_cases)
		datos_iniciales_seirs_1=[1-x[1]-x[2]*(1-x[1]), x[2]*(1-x[1]),x[1],0]
		parametros = x[3:6]
		prob     = ODEProblem(seirs!,datos_iniciales_seirs_1,(1,n),parametros)#,bstol=1e-7, reltol=1e-7)
		sol      = solve(prob,Vern7())
	    if any((!SciMLBase.successful_retcode(s.retcode) for s in sol)) # 
	        tot_loss = Inf
	    else
...
# ╔═╡ 2c360d41-2d6a-4687-88f3-40994dd662aa
optprob_seirs_1 = OptimizationProblem(func_seirs_1,[1/45.5e6,0.1, 0.1, 1.84, 0.6, 0.1],lb=[0., 0, 0, 0, 0,0],ub=[1000000/45.5e6, 0.5, 70, 10, 1,1.5])

# ╔═╡ 2e3d992e-0846-4d72-8760-15537c14c032
p_seirs_1 = solve(optprob_seirs_1,SAMIN(),maxiters=100000)

# ╔═╡ 052413ec-ba84-4420-9e97-38b5ff0351f8
begin
	datos_iniciales_seirs_1=[1-p_seirs_1[1]-p_seirs_1[2]*(1-p_seirs_1[1]), p_seirs_1[2]*(1-p_seirs_1[1]),p_seirs_1[1],0]
	prob_final_seirs_1 = ODEProblem(seirs!,datos_iniciales_seirs_1,(1,42),p_seirs_1[3:6])
	sol_opt_seirs_1    = solve(prob_final_seirs_1,Vern7())
	plot([1:42], getγEseirS(sol_opt_seirs_1, 42))
	scatter!([1:42],primera_ola.New_cases)
end

# ╔═╡ 56fd1728-7470-49ce-bfe5-d457222b2ac1
costo_seirs_1=func_seirs_1(vcat(datos_iniciales_seirs_1, p_seirs_1[3:6]), 0)

# ╔═╡ ff1f15af-9dc8-4d21-9171-4ee7bb2e0f61
md"""#### Para dos olas"""

# ╔═╡ ac52d23e-9392-40d4-a8fc-276f88fa0bf5
function func_seirs_2(x,r)
		tot_loss = 0.0
		n = length(segunda_ola.New_cases)
		datos_iniciales_seirs=[1-x[1]-x[2]*(1-x[1]), x[2]*(1-x[1]),x[1],0]
		parametros = x[3:6]
		prob     = ODEProblem(seirs!,datos_iniciales_seirs,(1,n),parametros)#,bstol=1e-7, reltol=1e-7)
		sol      = solve(prob,Vern7())
	    if any((!SciMLBase.successful_retcode(s.retcode) for s in sol)) # 
	        tot_loss = Inf
	    else
...
optprob_seirs_2 = OptimizationProblem(func_seirs_2,[1/45.5e6,0.1, 0.1, 1.84, 0.6, 0.1],lb=[0., 0, 0, 0, 0,0],ub=[1000000/45.5e6, 0.5, 70, 10, 1,1.5])

# ╔═╡ d7139c4f-18bf-405a-bbe9-78260ea81655
p_seirs_2 = solve(optprob_seirs_2,SAMIN(),maxiters=100000)

# ╔═╡ 640d8ef6-f592-4f82-94e3-198e792f5110
begin
	datos_iniciales_seirs_2=[1-p_seirs_2[1]-p_seirs_2[2]*(1-p_seirs_2[1]), p_seirs_2[2]*(1-p_seirs_2[1]),p_seirs_2[1],0]
	prob_final_seirs_2 = ODEProblem(seirs!,datos_iniciales_seirs_2,(1,52),p_seirs_2[3:6])
	sol_opt_seirs_2    = solve(prob_final_seirs_2,Vern7())
	plot([1:52], getγEseirS(sol_opt_seirs_2, 52))
	scatter!([1:52],segunda_ola.New_cases)
end

# ╔═╡ f0e8a514-08d5-4baf-bbd6-8ab662da6b7f
costo_seirs_2=func_seirs_2(vcat(datos_iniciales_seirs_2, p_seirs_2[3:6]), 0)

# ╔═╡ 44462ab3-038d-4151-9acd-d1619709cc8d
md""" De nuevo, a pesar de haber agregado variables y parámetros, la función costo no mejoró. Hace falta aclarar que dejamos los parametros "libres", es decir de 0 a ∞."""

# ╔═╡ 8a2e1da2-5e30-481b-8121-555e589bccb4
md""" $β^{-1}$ es mas baja que los anteriores modelos, este cambio para nosotro no tuvo sentido, debido a que nuestro objetivo era que el modelo pueda interpretar las 2 olas. 

Intuitivamente se deberia lograr con una correcta variacion entre los suceptibles y recuperados para que nuevamente puedan ser infectados, esto no se logra debido a que las olas estan muy "pegadas" nuestro modelo es muy simple para capturar este fenomeno, pues seguramente esta segunda ola se deba a otros aspectos, pudo haber sido por un evento social masivo que tuvo como consecuencia una segunda ola repentina.

Se puede llegar a una conclucion similar para los demas parametros del modelo."""
...
function func_separ(x,r)
		tot_loss = 0.0
		n = length(primera_ola.New_cases)
		datos_iniciales=[1-x[1]-x[2],(1-x[3])x[2],x[3]*x[2],(1-x[3])x[1],x[3]*x[1],0]
		parametros = x[4:7]
		prob     = ODEProblem(seir!,datos_iniciales,(1,n),parametros)#,bstol=1e-7, reltol=1e-7)
		sol      = solve(prob,Vern7())
	    if any((!SciMLBase.successful_retcode(s.retcode) for s in sol)) # 
...
# ╔═╡ ecb11d12-aa29-46f2-aae7-bb83e5692b60
begin
	datos_iniciales_separ=[1-p_separ[1]-p_separ[2],(1-p_separ[3])p_separ[2],p_separ[3]*p_separ[2],(1-p_separ[3])p_separ[1],p_separ[3]*p_separ[1],0]
	prob_final_separ = ODEProblem(separ!,datos_iniciales_separ,(1,42),p_separ[4:7])
	sol_opt_separ    = solve(prob_final_separ,Vern7())
	plot([1:42], getγEseparS(sol_opt_separ, 42))
	scatter!([1:42],primera_ola.New_cases)
end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "eeb2d20728e6739e9cc36f5132bcfe60bdd44705"
